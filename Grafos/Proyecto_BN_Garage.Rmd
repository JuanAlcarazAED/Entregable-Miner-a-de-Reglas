...
M.prob <- array(c(0.9, 0.1, 0.4, 0.6, 0.3, 0.7,
...
```{r Apartado 6}
#solucion
q1 <- querygrain(Garage, nodes=c("M"), type="marginal")
q1
```
...
which(q6 == max(q6), arr.ind = TRUE)
```

**Respuesta:** los más común es un coche viejo (`A = 3`, que significa `"Old"`), modelo A2, con kilometraje alto, al cual no hay que cambiarle ni las bujías ni los frenos. El resultado es algo contraintuitivo, pues era esperable que los coches de mayor kilometraje y más antiguos, cambien sus piezas de forma frecuente, debido al desgaste por el uso.
