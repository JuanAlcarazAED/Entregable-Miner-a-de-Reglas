...
- Según el test $\chi^2$ en todos los pares variables discretizadas hay dependencia entre ambas variables (o al menos hay evidencia estadística suficiente para creerlo), con la única excepción del par `Industrial.Risk` y `Credibility`, que es el único caso para el que el $p$-valor es menor que la significancia $0.05$. Y además se observa en el diagrama de mosaico que aproximadamente todas las clases están igualmente repartidas, lo cual apoya la hipótesis de la independencia.

- En cambio, según la V de Cramer, los pares de variables con mayor grado de asociación son los pares `Bankruptcy + Competitiveness` y `Bankruptcy + Financial.Flexibility`. Cosa que podemos confirmar visualmente en el diagrma de mosaico, donde por ejemplo podemos ver que las empresas menos competitivas acaban en bancarrota, mientras que aquellas que no llegaron a la bancarrota mostraron un nivel intermedio o alto de competitividad.
...
El modelo demuestra un ajuste excelente a los datos de test. La matriz de confusión del algoritmo CBA muestra que todas las predicciones, tanto de bancarrota como de no bancarrota, fueron correctas, alcanzando una sensibilidad y especificidad del 100% .

En la fase de entrenamiento, el modelo tan solo cometió un error (un falso negativo), prediciendo "NB" cuando en realidad era "B". En un entorno real, es fundamental minimizar este tipo de fallos: diagnosticar que una empresa está a salvo y que termine quebrando es un peor resultado que emitir una falsa alarma, ya que esta última al menos causaría que la empresa tome medidas preventivas.

Aun así, hay que ser cautelosos con estos modelos. La muestra es relativamente pequeña, con solo 250 empresas. Al exigir una confianza absoluta del 100%, estamos forzando al algoritmo a memorizar patrones muy específicos de este conjunto y causando un posible sobreajuste. Estos patrones serán difícilmente generalizables a la complejidad caótica del mercado real.

**Conclusiones del modelo basado en reglas**

¿has utilizado todas las variables en el modelo y por qué?

En el modelo CBA se usaron todas las variables para que el algoritmo tuviera la libertad de explorar todas las combinaciones posibles y descubrir cuáles son las asociaciones multivariantes más potentes en los datos.

¿has seleccionado reglas y por qué? ¿en base a qué criterio?

Las reglas del modelo CBA fueron filtradas y seleccionadas para evitar aquellas que fueran producto de la casualidad y garantizar así unas predicciones robustas. Los parámetros utilizados fueron:
- Soporte del 10% para obtener patrones frecuentes que representen a una parte significativa de las empresas.

- Confianza del 100% para asegurar que siempre que se cumplan las condiciones de la regla, el resultado sea exactamente el que se esperaba. 

- Longitud de regla mínima de 1 y máxima de 10 para evitar combinaciones excesivamente largas y mantener así una interpretación lógica y sencilla a ojos de una persona.